...
"""
Parses a python file to create a markdown (md) readme from any doc strings.

This file has been altered from the original source material: [automacdoc](https://github.com/AlexandreKempf/automacdoc)

**Changes:**
1. Added the ability to specify directories that you do not want the contain python files to be documented.

2. Changed the functionality to only document functions/classes defined in the module.

    * Originally it was documenting any imported python modules as well.

3. Added the module's overall doc string to be included in the md.

4. Various cosmetic changes to the output format.
"""
...
def rm_docstring_from_source(source):
    ...
    **Returns**
    ...
    source = "".join(source)
    # to handle indentation inside functions and classes
    source = source.split("\n")
...
def write_module(
    ...
    **Parameters**
    ...
    # Get all classes within this script.
    # Remove classes that were defined outside of this script.
    classes = []
    for name, obj in inspect.getmembers(module, inspect.isclass):
        if obj.__module__ == module_import:
            classes.append((name, obj))

    # Get all functions within this script.
    # Remove all functions that were defined outside of this script.
    functions = []
    ...
    main_doc = module.__doc__

    if main_doc is not None:
        md_file.writelines(main_doc)
        md_file.writelines("""\n______\n\n""")
...
def write_doc(src: str, mainfolder: str, skipdir: list):
    # variables
    ...
    doc_path = os.path.join(os.path.abspath(mainfolder), "docs", "src")
    ...
    while len(skipdir) > 0:
        ...
        # Get the first directory to skip.
        directory = skipdir.pop(0)

        for file in os.listdir(directory):
            abs_path = os.path.join(root_path, directory, file)

            # If a directory to be skipped contains a  sub-directory, add the sub-directory to the list of directories
            # to be skipped.
            if os.path.isdir(abs_path):
                skipdir.append(abs_path)

            # If a directory to be skipped contains a file, add that file to the list of files that should not be
            # commented.
            elif os.path.isfile(abs_path):
                skip_files.add(abs_path)

            # If there is an unknown case, print the file that caused this unknown case and exit.
            else:
                print("Unknown File: {}".format(abs_path))
                exit()
    ...
    list_glob = [
        p for p in full_list_glob
        if "/" + ign_pref_file not in p
